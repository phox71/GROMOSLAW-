...
class ToolTip:
    ...
    def show_tip(self, event=None):
        ...
        tk.Label(tw, text=self.text, justify=tk.LEFT, bg="#ffffe1", fg="black", relief=tk.SOLID, borderwidth=1, font=("Segoe UI", 8)).pack(ipadx=1)
...
class GamifikatorEditor:
    def __init__(self, root):
        self.root = root; self.root.title("GAMIFIKATOR 2026 - v2.3.2 [DATA INTEGRITY]")
        ...
        self.log("System Gamifikator v2.3.2 załadowany.")
    ...
    def handle_runtime_click(self, x, y):
        room = self.project.rooms[self.current_room_id]; target_x, target_y = x, y; hit_hp = None
        for hp in reversed(room["hotpoints"]):
            if hp.x <= x <= hp.x+hp.w and hp.y <= y <= hp.y+hp.h:
                hit_hp = hp; target_x, target_y = hp.x + hp.w//2, hp.y + hp.h; break
        polys = [Polygon([(p.points[j], p.points[j+1]) for j in range(0, len(p.points), 2)]) for p in room["walkable"]]
    ...
    def open_player_settings(self):
        ...
        def row(k):
            ...
            tk.Button(f, text="...", command=lambda: p_v.set(filedialog.askopenfilename() or p_v.get())).pack(side=tk.LEFT)
        ...
        def save():
            ...
            self.project.player["walk_speed"] = sp.get(); self.project.player["scale"] = sc.get(); self.refresh_canvas(); d.destroy()
    ...
    def draw_player(self, px, py, is_sel):
        ...
        path = anim_data["path"]; frames = anim_data.get("frames", 1)
        if path and os.path.exists(path):
            img = Image.open(path).convert("RGBA"); fw = img.width // frames; fh = img.height
    ...
    def on_canvas_down(self, event):
        ...
        if self.mode == "Select":
            ...
            for i, wa in enumerate(room["walkable"]):
                if min(wa.points[0::2]) <= x <= max(wa.points[0::2]): self.selected_object = {"type": "wa", "idx": i}; self.refresh_canvas(); return
    ...
    def on_canvas_right_click(self, event):
        ...
        if self.current_room_id:
            for hp in reversed(self.project.rooms[self.current_room_id]["hotpoints"]):
                if hp.x <= (event.x/self.view_scale) <= hp.x+hp.w and hp.y <= (event.y/self.view_scale) <= hp.y+hp.h: self.open_hp_settings(hp); return
    ...
    def refresh_obj_tree(self):
        ...
        if self.current_room_id:
            ...
            for i, hp in enumerate(r.get("hotpoints", [])): self.obj_tree.insert("", "end", iid=f"hp_{i}", text=hp.id, image=self.icons_ui.get("Edit_small"))
