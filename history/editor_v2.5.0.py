...
class GamifikatorEditor:
    ...
    def open_animatorator(self):
        ...
        hlbl(lp, "PODGLĄD ANIMACJI  (kliknij gdy eyedropper aktywny → kolor tła)")
        ...
        def on_pv_click(event):
            if not ey_on["v"]:
                return
            frame = st.get("current_pil")
            if frame is None:
                return
            scale = st["pv_scale"]; ox = st["pv_ox"]; oy = st["pv_oy"]
            ix = int((event.x - ox) / scale)
            iy = int((event.y - oy) / scale)
            if 0 <= ix < frame.width and 0 <= iy < frame.height:
                px = frame.getpixel((ix, iy))
            ...
            ey_on["v"] = False
            btn_ey.config(bg="#333", text="💧  EYEDROPPER  (kliknij podgląd)")
        pv_cv.bind("<Button-1>", on_pv_click)
        ...
        rhlbl("USUWANIE TŁA  (w pamięci — nie nadpisuje pliku źródłowego)")
        ey_row = tk.Frame(rp, bg="#0d0d0d"); ey_row.pack(fill=tk.X, padx=4, pady=(0, 2))
        btn_ey = tk.Button(ey_row, text="💧  EYEDROPPER  (kliknij podgląd)",
                           command=lambda: _toggle_ey(),
                           bg="#333", fg="white", relief=tk.FLAT, anchor="w")
        btn_ey.pack(side=tk.LEFT, fill=tk.X, expand=True)
        ey_swatch = tk.Label(ey_row, text="   ", bg="#262626", relief=tk.SOLID, bd=1, width=3)
        ey_swatch.pack(side=tk.LEFT, padx=4)
        ey_lbl = tk.Label(ey_row, text="—", bg="#0d0d0d", fg="#555", font=("Segoe UI", 8))
        ...
        tol_val_lbl = tk.Label(tol_row, text="30", bg="#0d0d0d", fg="white",
                               font=("Segoe UI", 8, "bold"), width=3)
        tol_val_lbl.pack(side=tk.RIGHT)
        tol_v = tk.IntVar(value=30)
        ...
        tk.Button(rp, text="🗑  REMOVE BG  (zastosuj i ponownie wykryj klatki)",
        ...
        def _toggle_ey():
            ey_on["v"] = not ey_on["v"]
            if ey_on["v"]:
                btn_ey.config(bg="#7B2FBE", text="💧  EYEDROPPER  [AKTYWNY — kliknij podgląd]")
            else:
                btn_ey.config(bg="#333", text="💧  EYEDROPPER  (kliknij podgląd)")
        ...
        def do_remove_bg_anim():
            ...
            if not bg_color["v"]:
                self._toast("Użyj eyedroppera na podglądzie klatki.", "warn"); return
            data = np.array(st["img"])
            r0, g0, b0 = bg_color["v"]
            diff = data[:, :, :3].astype(float) - np.array([r0, g0, b0], float)
            dist = np.sqrt((diff ** 2).sum(axis=2))
            data[dist <= (tol_v.get() * 2.55), 3] = 0
            st["img"] = Image.fromarray(data)
            status_v.set(f"Remove BG zastosowano (tol={tol_v.get()}). Ponownie wykrywam klatki…")
            do_detect()
