...
class GamifikatorEditor:
    def __init__(self, root):
        ...
        self.engine_loop()
        self.log("System Gamifikator v2.4.6 zaladowany.")
    ...
    def setup_ui(self):
        ...
        tk.Button(self.toolbar, text="SPRITES",     command=self.open_sprite_library,
                  bg="#C4A35A", fg="black", relief=tk.FLAT, font=("Segoe UI", 9, "bold")).pack(side=tk.LEFT, padx=10)
        ...
        for m in ["Select", "Walkable", "Hotpoint"]:
            btn = tk.Button(self.tool_frame, image=self.icons_ui.get(f"{m}_dark"),
                            bg="#1e1e1e", relief=tk.FLAT, command=lambda mode=m: self.set_mode(mode))
            btn.pack(pady=(15, 0), padx=10)
        ...
        self.room_listbox.pack(fill=tk.X, padx=10, pady=5)
        self.room_listbox.bind("<<ListboxSelect>>", self.on_room_selected)
        tk.Button(self.sidebar, text="+ NEW SCENE", command=self.add_room,
                  bg="#333", fg="white", relief=tk.FLAT).pack(fill=tk.X, padx=10)
        self.obj_tree = ttk.Treeview(self.sidebar, show="tree", height=10)
        self.obj_tree.pack(fill=tk.X, padx=10, pady=5)
    ...
    def open_sprite_library(self):
        if not self.project:
            messagebox.showwarning("Brak projektu", "Najpierw stwórz lub otwórz projekt.")
            return
        ...
        def do_trim():
            sid = sel["sid"]
            if not sid:
                messagebox.showwarning("Brak wyboru", "Wybierz sprite z listy.")
                return
        ...
        def do_remove_bg():
            ...
            if not sid:
                ...
                return
            if not picked["color"]:
                messagebox.showwarning("Brak koloru", "Użyj eyedroppera, żeby pobrać kolor tła.")
                return
            path = self.project.sprites[sid]["path"]
    ...
    def open_animatorator(self):
        ...
        lb_f = tk.Frame(rp, bg="#0d0d0d"); lb_f.pack(fill=tk.X, padx=4, pady=2)
        lib_lb = tk.Listbox(lb_f, bg="#1a1a1a", fg="#ddd", height=5, borderwidth=0,
                            selectbackground="#7B2FBE", font=("Segoe UI", 9), exportselection=False)
        lib_lb.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        tk.Button(lb_f, text="🗑", command=lambda: del_from_lib(),
                  bg="#8b0000", fg="white", relief=tk.FLAT, width=3).pack(side=tk.LEFT, padx=2)
        ...
        def do_detect():
            if st["img"] is None:
                messagebox.showwarning("Brak obrazu", "Najpierw załaduj sprite sheet.")
                return
            img = st["img"]
            bbox = img.split()[3].getbbox()
            if bbox:
        ...
        def toggle_play():
            if not st["uniform"]:
                messagebox.showwarning("Brak klatek", "Wykryj klatki przed odtworzeniem.")
                return
        ...
        def save_anim():
            if not st["uniform"]:
                messagebox.showwarning("Brak klatek", "Wykryj klatki przed zapisem.")
                return
            ...
            messagebox.showinfo("Zapisano",
                f"Animacja '{name}' zapisana w bibliotece!\n"
                f"Plik: {os.path.basename(out_path)}\n"
                f"Klatki: {n}  |  Rozmiar klatki: {fw}×{fh}px\n"
                f"Gotowy do użycia w CONFIGURE PLAYER → animations")

        def del_from_lib():
            sel_idx = lib_lb.curselection()
            if not sel_idx:
                return
            if not hasattr(self.project, "animations"):
                return
            keys = list(self.project.animations.keys())
            if sel_idx[0] < len(keys):
                del self.project.animations[keys[sel_idx[0]]]
                refresh_lib()
        ...
        def refresh_lib():
            ...
            if not hasattr(self.project, "animations"):
                return
            for a in self.project.animations.values():
                lib_lb.insert(tk.END,
                    f"  {a['name']}  ({a['frames']} klatek | {a['frame_w']}×{a['frame_h']}px | {a['fps']} fps)")
    ...
    def open_scripts(self):
        ...
        def save_script():
            sid = sel_id["v"]
            if not sid:
                messagebox.showwarning("Brak wyboru", "Utwórz lub wybierz skrypt z listy.")
                return
        ...
        def run_test():
            ...
            try:
                ...
                messagebox.showerror("Błąd skryptu", str(e))
    ...
    def on_canvas_down(self, event):
        ...
        for i, raw in enumerate(reversed(room["hotpoints"])):
            ...
            if hp.x+hp.w-15 <= x <= hp.x+hp.w+15 and hp.y+hp.h-15 <= y <= hp.y+hp.h+15:
                self.drag_data = {"mode": "resize", "obj": hp,
                                  "start_w": hp.w, "start_h": hp.h, "start_x": x, "start_y": y}
    ...
    def on_canvas_drag(self, event):
        ...
        if mode == "move_player":
            ...
            if hasattr(o, 'image_path') and o.image_path and os.path.exists(o.image_path):
                raw_img = Image.open(o.image_path)
                o.scale_png = int((o.w / raw_img.width) * 100)
        self.refresh_canvas()
    ...
    def _draw_player_placeholder(self, px, py):
        vs = self.view_scale
        self.canvas.create_oval((px-10)*vs, (py-10)*vs, (px+10)*vs, (py+10)*vs, fill="white")
    ...
    def _draw_dialogs(self, px, py):
        for dg in self.active_dialogs:
            if dg["owner"] == "p":
                ...
                tx, ty = self.get_hp_center(dg["hp_id"])
            self.canvas.create_text(
                tx * self.view_scale, ty * self.view_scale,
                text=dg["text"], fill="white",
                font=("Segoe UI", 12, "bold"), justify="center", width=300)
    ...
    def set_mode(self, mode):
        ...
        for m, btn in self.tool_btns.items():
            btn.config(image=self.icons_ui.get(f"{m}_light" if m == mode else f"{m}_dark"))
